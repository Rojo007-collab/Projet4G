function res = parcours_hamiltonian2_misorder_inv(in)
    [W H] = size(in);
...
%%%    disp( [hamiltonian2XX; hamiltonian2YY]);
	...
	for i=1:W  % misorder this boucle
		for j = 1:H % misorder this boucle
			indexx = (j-1)*H+i;
			hamiltonian2X(indexx) = tempX(p);
			hamiltonian2Y(indexx) = tempY(p);
			p = p+1;
		end
    end
%    plot(hamiltonian2X, hamiltonian2Y);
  ...
  %          disp([i_permute j_permute])
            res(hamiltonian2Y(i), hamiltonian2X(i)) = in(i_permute, j_permute);
